...
from mms.models.wallet_models import (
    TokenProfile,
    UserSignUp,
...
from mms.routers.routes_utils import get_refresh_token
...
async def user_signup(user_signup: UserSignUp):
    return await wallet_service.user_signup(user_signup=user_signup)


@api_router.post("/login", response_model=AuthenticatedUserProfile)
async def login_user(login: Login):
    return await wallet_service.login_user(login=login)


@api_router.post("/forgot/password", response_model=TokenProfile)
async def forgot_password(email: AnEmail):
    return await wallet_service.forgot_password(email=email)


@api_router.post("/change/password")
async def change_password(password_change: PasswordChange):
    return await wallet_service.change_password(password_change=password_change)


@api_router.post("/token/verification")
async def token_verification(token_verification: TokenVerification):
    print('heeee')
    return await wallet_service.token_verification(token_verification=token_verification)


@api_router.post("/logout")
async def logout():
    return await wallet_service.logout()
...
async def refresh(refresh_token: AnRefreshToken):
    return await wallet_service.refresh(refresh_token=refresh_token)
